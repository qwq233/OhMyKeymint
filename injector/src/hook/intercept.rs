...
static PENDING_SYNTHETIC_TRANSACTION_COMPLETIONS: LazyLock<
...
unsafe fn parse_write_buffer(ptr: *mut c_void, size: u64) {
            ...
            match cmd_nr {
                BC_TRANSACTION_NR | BC_REPLY_NR | BC_TRANSACTION_SG_NR | BC_REPLY_SG_NR => {
                    ...
                    if cmd_size == expected_size {
                        ...
                        if matches!(cmd_nr, BC_REPLY_NR | BC_REPLY_SG_NR) {
            ...
            match cmd_nr {
                    ...
                    if cmd_size == size_of::<binder_ptr_cookie>() {
                        ...
                        if handle_synthetic_ref_command(target, cmd_nr) {
                            let submitted = match cmd_nr {
                                BR_INCREFS_NR => submit_synthetic_command(
                                    fd,
                                    old_ioctl_fn,
                                    BC_INCREFS_DONE_CMD,
                                    ptr_cookie,
                                ...
                                BR_ACQUIRE_NR => submit_synthetic_command(
                                    fd,
                                    old_ioctl_fn,
                                    BC_ACQUIRE_DONE_CMD,
                                    ptr_cookie,
                ...
                BR_ATTEMPT_ACQUIRE_NR => {
                    if cmd_size == size_of::<binder_pri_ptr_cookie>() {
                        ...
                        if handle_synthetic_ref_command(target, cmd_nr) {
                            let submitted = submit_synthetic_command(
                                fd,
                                old_ioctl_fn,
                ...
                | BR_CLEAR_FREEZE_NOTIFICATION_DONE_NR => {
                    if cmd_size == size_of::<libc::c_ulong>() {
                        let cookie = std::ptr::read_unaligned(payload as *const libc::c_ulong);
                        let target = LocalBinderTarget {
                            ptr: cookie,
                            cookie: 0,
                        };
                ...
                BR_FROZEN_BINDER_NR => {
                    if cmd_size == size_of::<binder_frozen_state_info>() {
                        let frozen =
                            std::ptr::read_unaligned(payload as *const binder_frozen_state_info);
                        let target = LocalBinderTarget {
                            ptr: frozen.cookie,
                            cookie: 0,
                        };
                        if handle_synthetic_ref_command(target, cmd_nr) {
                            let submitted = submit_synthetic_command(
                                fd,
                                old_ioctl_fn,
                                BC_FREEZE_NOTIFICATION_DONE_CMD,
                                frozen.cookie,
